...
public class BitwiseXORofAllPairings {
	public int xorAllNums(int[] nums1, int[] nums2) {
		...
		// Because if nums2 has an odd number of elements, each element in nums1 will be represented an odd number
		// of times when taking the XOR with all elements of nums2 and vice versa.
		if (nums2.length % 2 == 1) {
			for (int value : nums1) {
				result ^= value;
			}
